...
public class Sms extends CordovaPlugin {
				...
				public void run() {
					try {
						...
						if (android.os.Build.MANUFACTURER.equalsIgnoreCase("Samsung")) {
							// See
							// http://stackoverflow.com/questions/18974898/send-sms-through-intent-to-multiple-phone-numbers/18975676#18975676
							separator = ",";
						}
						...
						// replacing \n by new line if the parameter
						// replaceLineBreaks is set to true
						if (replaceLineBreaks) {
							message = message.replace("\\n", System.getProperty("line.separator"));
						}
						if (!checkSupport()) {
							callbackContext.sendPluginResult(
									new PluginResult(PluginResult.Status.ERROR, "SMS not supported on this platform"));
							return;
						}
	...
	private void send(final CallbackContext callbackContext, String phoneNumber, String message) {
		...
		// by creating this broadcast receiver we can check whether or not the
		// SMS was sent
		final BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {

			boolean anyError = false; // use to detect if one of the parts
										// failed
		...
		PendingIntent sentIntent = PendingIntent.getBroadcast(this.cordova.getActivity(), 0,
				new Intent(intentFilterAction), 0);

		// depending on the number of parts we send a text message or multi
		// parts
		if (parts.size() > 1) {
	...
	private JSONObject getJsonFromSmsMessage(SmsMessage sms) {
		...
		try {
			...
			json.put(BODY, sms.getMessageBody()); // May need
